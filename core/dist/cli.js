...
import { wsAnalyze, wsBeginDetailed, wsBranchAssign, wsBranchAdopt, wsBranchBegin, wsBranchRollback, wsBranchUpdate, wsBranchCommit, wsBranchDiscard, wsBranchMerge, wsCommit, wsDeleteCopy, wsDiff, wsInit, wsListVersions, wsMigrateLegacy, wsParallelView, wsReadDoc, wsRestoreCopy, wsRollback, wsStatus, wsTempApply, wsTempDiscard, wsTempSave, resolveRepo, } from "./workspace.js";
    ...
    for (let i = 1; i < argv.length; i++) {
        ...
        if (t.startsWith("--")) {
            ...
            if (next === undefined || next.startsWith("--"))
                ...
                a[key] = next;
                if (key === "exclude")
                    excludes.push(next); // --exclude 可重复,逐条累积
                i++;
            }
        }
    }
...
async function main() {
    ...
    const src = process.env.PVC_CLIENT?.trim()
    ...
    switch (cmd) {
        case "init":
            ...
            break;
        case "begin": {
            const r = await wsBeginDetailed(dir, {
        ...
        case "versions": {
            ...
            out = list
                .map((v) => `${v.code} [${v.source === "gui" ? "人工提交" : v.source === "api" || v.source.startsWith("mcp") || v.source === "cli" ? "ZCode提交" : v.source}]${v.materialized ? " 已物化副本" : ""} ${v.date}\n  需求:${v.purpose}\n  改动:${v.changes}`)
                .join("\n");
            break;
        }
        case "log":
            ...
            break;
        case "read":
            out = await wsReadDoc(dir, String(a.code ?? ""), a.which ?? "purpose");
            break;
        case "diff":
            out =
                (await wsDiff(dir, String(a.from ?? ""), a.to ? String(a.to) : null, a.mode === "full" ? 500 : 100000)) || "(无差异)";
            break;
        case "analyze": {
            const r = await wsAnalyze(path.resolve(String(a.dir ?? process.cwd())));
            const kb = (n) => (n >= 1024 * 1024 ? (n / 1048576).toFixed(1) + "MB" : Math.ceil(n / 1024) + "KB");
            out = [
                `文件夹:${r.dir}`,
        ...
        case "rollback":
            out = await wsRollback(dir, {
                code: String(a.code ?? ""),
                reason: a.reason ? String(a.reason) : undefined,
                discardDirty: bool(a["discard-dirty"] ?? a.discardDirty),
                resolveInProgress: String(a["resolve-in-progress"] ?? a.resolveInProgress ?? "") === "stash"
                    ? "stash"
                    : undefined,
                source: src,
            });
        ...
        case "restore-copy":
            ...
            break;
        case "migrate":
            out = await wsMigrateLegacy(path.resolve(String(a.new ?? "")), { title: String(a.title ?? ""), oldRepo: path.resolve(String(a.old ?? "")), source: src });
            break;
        case "branch-begin": {
            const r = await wsBranchBegin(dir, {
                requirement: String(a.requirement ?? ""),
        ...
        case "branch-assign":
            ...
            break;
        case "branch-adopt": {
            const r = await wsBranchAdopt(dir, {
                name: String(a.name ?? ""),
                requirement: String(a.requirement ?? ""),
                assignee: a.assignee ? String(a.assignee) : undefined,
            }, src);
        ...
        case "branch-rollback":
            out = await wsBranchRollback(dir, {
                name: String(a.name ?? ""),
            ...
            break;
        case "branch-update":
            out = await wsBranchUpdate(dir, { name: String(a.name ?? ""), token: a.token ? String(a.token) : undefined }, src);
            break;
        case "branch-merge":
            out = await wsBranchMerge(dir, {
                name: String(a.name ?? ""),
                token: a.token ? String(a.token) : undefined,
