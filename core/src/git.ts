...
export async function gitBuffer(
  ...
  return new Promise((resolve, reject) => {
    execFile(
      ...
      { cwd: repo, maxBuffer: MAX_BUFFER, windowsHide: true },
      (err, stdout) => {
        if (err) reject(err);
...
export function parsePorcelainZ(out: string): StatusEntry[] {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const entry: StatusEntry = { xy, path };
    if (xy[0] === "R" || xy[0] === "C") {
...
export async function head(repo: string): Promise<string> {
...
export async function statusPorcelain(
  ...
  return parsePorcelainZ(out).filter((e) =>
    positives.some(
      (p) =>
        p === "." ||
        p.includes("*") ||
...
export async function addPaths(repo: string, paths: string[]): Promise<void> {
  if (paths.length === 0) return;
  const positives = paths.filter((p) => !p.startsWith(":"));
...
export async function lsTreeFiles(
  repo: string,
  ref: string,
  pathspec?: string,
): Promise<string[]> {
...
export async function filterExistingPathspecs(
  repo: string,
  positives: string[],
): Promise<string[]> {
  const out: string[] = [];
...
export async function tagExists(repo: string, name: string): Promise<boolean> {
  const out = await git(repo, ["tag", "-l", name]);
  return out.trim() === name;
}

export async function tagList(repo: string): Promise<string[]> {
...
export async function diffFull(
  ...
  if (lines.length <= maxLines) return out.trimEnd();
  return (
