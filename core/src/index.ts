 ...
 */
export function resolveRepoFor(
  repoPath: string | undefined,
  ctx: ServerContext,
  env: NodeJS.ProcessEnv = process.env,
): { repo: string; warn: string } {
  const explicit = repoPath?.trim();
  const candidate = explicit || ctx.defaultRepo?.trim() || env.PAPER_VERSION_REPO?.trim();
  if (!candidate) {
    throw new Error(
      "缺少 repoPath:请传入当前项目文件夹的绝对路径(多项目环境下必须显式指定;若本工具已在桌面端绑定默认项目,不传即自动路由)",
    );
  }
  ...
  if (explicit && ctx.defaultRepo?.trim() && !sameRepo(explicit, ctx.defaultRepo)) {
    ...
    if (ctx.strict) {
      throw new Error(
        `严格路由:工具「${who}」已指定项目 ${bound},本次调用目标 ${repo} 超出绑定,已拒绝。` +
...
export function createServer(source: AuditSource = "mcp-stdio", ctx: ServerContext = {}) {
  const server = new McpServer({
    name: "vcs-workspace",
    version: "1.0.0",
  });
  const tag: AuditTag = normTag({ source, client: ctx.client });
  const clientLabel = ctx.client ?? "未命名工具";

  const R = (repoPath?: string): string => resolveRepoFor(repoPath, ctx, process.env).repo;
  ...
  const repoOpt = {
    repoPath: z
      ...
      .optional()
      .describe(
        "工作区文件夹(绝对路径);解析顺序:显式参数 > 路由表绑定的默认项目 > PAPER_VERSION_REPO 环境变量",
      ),
  };
  ...
  function reg(
    ...
    server.registerTool(name, spec as never, async (args: any): Promise<any> => {
      if (ctx.permission === "readonly" && WRITE_TOOLS.has(name)) {
        throw new Error(
          `工具「${clientLabel}」以只读模式接入(readonly),写操作 ${name} 已拒绝;` +
      ...
      if (ctx.defaultRepo && args && typeof args === "object" && "repoPath" in args) {
        try {
          const { warn } = resolveRepoFor(args.repoPath as string | undefined, ctx, process.env);
  ...
  reg(
    "vcs_status",
    {
      title: "工作区版本状态",
      description:
        "会话第一步必须调用。返回工作区(一个项目文件夹,内含 main/ 项目主体、V X.Y.Z/ 版本子文件夹、工作区日志.md)的当前状态:最新版本/进行中版本(含来源与最近活跃)/工作区变更/越界文件。只信本工具实况,禁止凭记忆断言版本。若尚不是工作区,先 vcs_init。",
      inputSchema: {
        ...repoOpt,
        sessionToken: z
          .string()
    ...
    async ({ repoPath, sessionToken }) => {
      ...
      if (sessionToken) await wsTouch(repo, sessionToken, tag).catch(() => false);
  ...
  reg(
    "vcs_init",
    {
      title: "创建工作区",
      description:
        "把一个文件夹初始化为受管工作区:自动 git init(若需要)并创建 main/(项目主体,用户与 AI 只在此修改)、工作区日志.md、.vcs/(工具状态)。此后 AI 只允许产生这三类产物,越界文件会被 vcs_commit 拒绝。",
  ...
  reg(
    "vcs_begin",
    {
      title: "开始新版本(记录用户需求)",
      description:
        "每次用户提出修改需求时必须先调用:记录用户需求并锁定基线,签发会话令牌(收尾 vcs_commit 时经 sessionToken 传回;令牌丢失可用 takeover:\"continue\" 续作)。之后只在 main/ 内修改;禁止在工作区根目录产生 main/、V*、工作区日志.md、.vcs/ 之外的任何文件。若另一会话已有进行中版本:不要替它收尾或回滚,等待其完成;确认其已停止后用 takeover:\"continue\"(沿用原需求与半成品)或 takeover:\"fresh\"(半成品 stash 快照后按新需求开新版本)显式接管。",
      inputSchema: {
        ...repoOpt,
  ...
  reg(
    "vcs_commit",
    {
      ...
      description:
        "修改完成后必须调用:按级别定版 V X.Y.Z → 生成 V 文件夹(更新目的.md=用户需求 + 改动日志.md=相对上一版改动)→ 追加工作区日志.md → git 提交+tag。bump:content=功能迭代(默认)/polish=微调/major=里程碑。必须携带本会话 vcs_begin 签发的会话令牌(sessionToken);替其他会话收尾会被拒绝。越界文件或空改动会被拒绝。",
      inputSchema: {
        ...repoOpt,
  ...
  reg(
    "vcs_rollback",
    {
      title: "回滚到指定版本",
      description:
        "把 main/ 回滚到指定 V 版本的内容,并将该版本完整项目副本物化到其 V 文件夹(项目副本/)。回滚本身登记为新版本。历史永不改写。工作区有未收尾修改需 discardDirty:true;存在进行中版本时须 resolveInProgress:\"stash\"(半成品快照进 git stash,不丢失)。",
      inputSchema: {
        ...repoOpt,
  ...
  reg(
    "vcs_delete_copy",
    {
      title: "删除版本副本",
      description:
        "删除回滚时物化在 V<版本>/项目副本/ 的完整项目副本(副本管理)。版本历史、tag 与 main/ 均不受影响:该版本内容仍保存在 git tag 中,随时可用 vcs_restore_copy(或 GUI「恢复副本」)重新物化。用于回滚试验结束后清理占空间的副本。",
      inputSchema: {
        ...repoOpt,
  ...
  reg(
    "vcs_restore_copy",
    {
      title: "生成版本副本",
      description:
        "为任意版本生成完整项目副本到 V<版本>/项目副本/(浏览用快照,内容取自版本历史,不影响 main/ 与 tag)。已有副本时拒绝(先删再建);删除用 vcs_delete_copy。",
      inputSchema: {
        ...repoOpt,
        code: z.string().describe("目标版本号,如 V1.10.0"),
      },
  ...
  reg(
    "vcs_diff",
    {
      title: "版本间差异",
      description:
        "对比 main/(项目主体)在两个版本之间的差异。from 必填(V 编号或 git tag);to 缺省=当前 main/ 工作区。mode=stat 统计,full 完整 diff(≤500 行)。",
      inputSchema: {
        ...repoOpt,
  ...
  reg(
    "vcs_audit",
    {
      title: "操作审计流水",
      description:
        "返回最近 N 条工作区操作审计(时间|来源|操作|结果;来源为 传输通道:工具名,如 mcp-http:claude)。",
    ...
    async ({ repoPath, limit }) => {
      ...
      try {
        const raw = await readFile(
          path.join(R(repoPath), ".vcs", "audit.log"),
          "utf8",
        );
  ...
  reg(
    "vcs_migrate",
    {
      title: "迁移旧论文库",
      description:
        "把旧模型论文库(paper_versions/,CN/EN/TEX 编号)迁移为 v3 工作区:现役文件→main/,历史→V0.i.0 存档(注明原编号);旧库原样保留。",
      inputSchema: {
    ...
    async ({ newRepoPath, title, oldRepoPath }) => ({
      content: [
        {
          type: "text",
          text: await wsMigrateLegacy(
            path.resolve(newRepoPath),
            { title, oldRepo: path.resolve(oldRepoPath), source: tag },
          ),
        },
      ],
    }),
  ...
  reg(
    "vcs_analyze",
    {
      title: "深入分析文件夹",
      description:
        "对任意文件夹做深入分析与整理方案:结构与体量盘点、是否已是工作区、转为标准工作区(main/ + V 版本 + 工作区日志)的整理步骤。用户说『对此工作区进行版本管理』时先调用本工具分析,再按方案执行 vcs_init。",
      inputSchema: {
  ...
  reg(
    "vcs_branches",
    {
      title: "并行分支与 temp 清单",
      description:
        "列出工作区的并行分支(名称/需求/指派工具/领先提交数/工作树绝对路径/最近活跃)、待处理 temp 条目与合并冲突状态。用户提及任何分支操作(创建/删除/在分支上变更/合并)前先调本工具看现状;begin 被拒(另一会话占用 main/)或需要处理 temp 时也先调它。",
      inputSchema: repoOpt,
    },
    async ({ repoPath }) => ({
      content: [
  ...
  reg(
    "vcs_branch_commit",
    {
      title: "分支检查点提交",
      description:
        "在并行分支工作树内做检查点提交(普通 git 提交,不占 V 版本号、不生成 V 文件夹)。当用户说「在分支 xx 上变更/提交/存个检查点」时使用。分支属主可调用:令牌持有者、指派工具(assignedClient)、GUI 用户;其他工具默认放行+提醒,strict 档拒绝。",
      inputSchema: {
        ...repoOpt,
  ...
  reg(
    "vcs_branch_assign",
    {
      title: "改派分支管理工具",
      description:
        "改派或清空并行分支的管理工具(assignedClient)。当用户说「把这个分支交给 XX 工具」「换 XX 管这个分支」时使用;assignee 留空=清除指派。分支属主可调用。",
      inputSchema: {
        ...repoOpt,
  ...
  reg(
    "vcs_branch_rollback",
    {
      title: "分支回退检查点",
      description:
        "把并行分支的工作树回退到某个检查点(缺省回退上一检查点;to 支持 1 基序号或 sha 前缀)。当用户说「分支回退/撤销分支上最后一次提交/回到分支的第 N 个检查点」时使用。工作树有未提交修改时默认拒绝(force 确认放弃)。分支属主可调用。",
      inputSchema: {
        ...repoOpt,
        name: z.string().describe("分支短名"),
  ...
  reg(
    "vcs_branch_update",
    {
      title: "主干合入分支",
      description:
        "把 main/ 最新内容合入并行分支,对齐主干。当用户说「把主干最新合到分支 xx」「分支对齐主干」时使用;分支基点之后主干发生过回滚时,先用它对齐再合并。工作树有未提交修改时拒绝。分支属主可调用。",
      inputSchema: {
        ...repoOpt,
        name: z.string().describe("分支短名"),
        token: z.string().optional().describe("会话令牌(属主可省略)"),
  ...
  reg(
    "vcs_branch_adopt",
    {
      title: "导入并行分支",
      description:
        "把仓库现存但未登记的 vcs/task/* 分支(孤儿分支,如历史清理中断残留)重建工作树并纳入管理。当用户说「导入分支」时使用;先用 vcs_branches 查看 orphans 清单。requirement 必填(记录该分支的用户需求)。",
      inputSchema: {
        ...repoOpt,
    ...
    async ({ repoPath, ...rest }) => {
      const r = await wsBranchAdopt(R(repoPath), rest as { name: string; requirement: string; assignee?: string }, tag);
      return { content: [{ type: "text", text: r.message }] };
    },
  );
  ...
  reg(
    "vcs_branch_merge",
    {
      title: "合并并行分支(定版)",
      description:
        "把并行分支合并进 main/ 并生成一个 V 版本(一个需求一个版本)。当用户说「合并分支 xx」「分支收版」时使用。前置:主工作区无进行中版本、无未收尾修改(合并点串行化)。产生冲突时返回冲突文件清单并挂起:在 main/ 内修复冲突标记后再次调用本工具完成定版,或传 abort:true 干净放弃(分支保留)。分支属主可调用(令牌/指派工具/GUI)。",
      inputSchema: {
        ...repoOpt,
        name: z.string().describe("分支短名"),
  ...
  reg(
    "vcs_branch_discard",
    {
      title: "丢弃并行分支",
      description:
        "删除并行分支的工作树与分支 ref(未合并的修改不可恢复,写审计)。当用户说「删除/删减分支 xx」「不要这个分支了」时使用——先向用户确认再调。工作树有未提交修改时默认拒绝(force:true 确认放弃);该分支处于合并冲突挂起期会先干净回退合并。分支属主可调用(令牌/指派工具/GUI)。",
      inputSchema: {
        ...repoOpt,
        name: z.string().describe("分支短名"),
        token: z.string().optional().describe("vcs_branch_begin 签发的会话令牌(被指派工具/GUI 可省略)"),
  ...
  reg(
    "vcs_temp_save",
    {
      ...
      description:
        "补救通道:发现项目被锁/越界无法清理/其他原因无法提交时,把 main/ 里已产生的变化复制保全到工作区根目录 temp/<name>/(含 manifest 与说明.md),随后 main/ 恢复干净——变化不丢、工作区解堵。之后由用户决定 vcs_temp_apply 恢复应用或 vcs_temp_discard 丢弃。注意:有进行中版本时仅持有者可调用(会同时放弃该任务);合并冲突处理期间不可用。",
      inputSchema: {
        ...repoOpt,
  ...
  reg(
    "vcs_temp_apply",
    {
      ...
      description:
        "把 temp/<name>/ 保存的变化写回 main/(按 manifest 恢复文件与删除)。应用后变更未登记,需走 vcs_begin → vcs_commit 定版。",
      inputSchema: {
        ...repoOpt,
...
export async function startServer(source: AuditSource = "mcp-stdio"): Promise<void> {
  const ctx: ServerContext = {
    client: process.env.PVC_CLIENT?.trim() || undefined,
    permission:
      (process.env.PVC_PERMISSION ?? "").trim().toLowerCase() === "readonly" ? "readonly" : "auto",
