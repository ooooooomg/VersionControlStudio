...
export function wsCodeOf(round: number, content: number, polish: number): string {
...
export function isAllowedPath(rel: string): boolean {
  ...
  if (p === ".gitignore" || p === ".gitattributes" || p === ".zcodeignore") return true; // 仓库机械文件(含 ZCode 会话忽略表)
  if (p === WS_LOG || p.startsWith(WS_DIR + "/") || p === WS_DIR) return true;
  if (p === WS_MAIN || p.startsWith(WS_MAIN + "/")) return true;
  if (p === WS_TEMP_DIR || p.startsWith(WS_TEMP_DIR + "/")) return true;
  return /^V\d+\.\d+\.\d+\//.test(p) || /^V\d+\.\d+\.\d+$/.test(p);
}

export async function wsLoadRegistry(repo: string): Promise<WsRegistry | null> {
  try {
    return JSON.parse(
      await fs.readFile(path.join(repo, WS_REGISTRY), "utf8"),
    ) as WsRegistry;
  } catch {
    return null;
  }
...
async function wsSaveRegistry(repo: string, reg: WsRegistry): Promise<void> {
...
export async function wsAppendAudit(
  repo: string,
  tag: SourceLike,
  op: string,
  detail: string,
  ...
  await fs.appendFile(
    path.join(repo, WS_AUDIT),
    `${ts} | ${tagLabel(tag)} | ${op} | ${detail.replaceAll("\n", " ⏎ ").slice(0, 400)}\n`,
    "utf8",
  );
}

/** 分支工作树守卫(V1.28.0):worktree 检出自带过期 registry 快照,会被误当独立工作区(嵌套分支/幽灵定版/tag 污染) */
async function assertNotBranchWorktree(repo: string): Promise<void> {
  const norm = repo.replaceAll("\\", "/");
  if (norm.includes(`${WS_BRANCH_DIR}/`)) {
    throw new Error(
      `${repo} 是并行分支的工作树,不能作为工作区操作:请改用主工作区根目录路径` +
  ...
  try {
    ...
    if (st.isFile()) {
      throw new Error(
        `${repo} 是 git 链接工作树(疑似并行分支工作树),不能作为工作区操作:请改用主工作区根目录路径`,
      );
    }
  } catch (err) {
    if (err instanceof Error && err.message.includes("工作区操作")) throw err;
    /* .git 不存在等:交由后续 registry 检查报错 */
  }
}

async function wsRequire(repo: string): Promise<WsRegistry> {
  await assertNotBranchWorktree(repo);
  const reg = await wsLoadRegistry(repo);
  if (!reg) {
    throw new Error(
      `${repo} 不是本工具的工作区:请先调用 vcs_init 创建工作区结构(main/ + 工作区日志.md + .vcs/)`,
    );
  }
  return reg;
}
...
export async function wsOffenders(repo: string): Promise<string[]> {
...
function nextCode(last: WsVersion | null, bump: Bump | "major") {
  const parse = (code: string) => {
    const m = /^V(\d+)\.(\d+)\.(\d+)$/.exec(code)!;
    return { round: Number(m[1]), content: Number(m[2]), polish: Number(m[3]) };
  };
  if (!last) return "V1.0.0"; // 初始版本一律 V1.0.0
  const v = parse(last.code);
  if (bump === "major") return `V${v.round + 1}.0.0`;
  if (bump === "polish") return `V${v.round}.${v.content}.${v.polish + 1}`;
...
const heldWriteLocks = new Map<string, { depth: number; timer: NodeJS.Timeout }>();

function lockKey(repo: string): string {
...
function pidAlive(pid: number): boolean {
  try {
    ...
    return (err as NodeJS.ErrnoException)?.code === "EPERM"; // EPERM=存在但属其他用户
  }
}

async function readLockInfo(repo: string): Promise<WriteLockInfo | null> {
  try {
    return JSON.parse(await fs.readFile(path.join(repo, WS_WRITE_LOCK), "utf8")) as WriteLockInfo;
  } catch {
    return null; // 不存在或半截 JSON
  }
}

async function lockAgeMs(repo: string): Promise<number> {
  try {
...
async function ensureGitExclude(repo: string): Promise<void> {
...
async function acquireWriteLock(repo: string, source: string): Promise<void> {
  ...
  for (;;) {
    try {
      ...
      const info: WriteLockInfo = { pid: process.pid, source, acquiredAt: nowStr() };
    ...
    if (stale) {
      ...
      await wsAppendAudit(
        ...
        "ws_lock_takeover",
        `holder_pid=${info?.pid ?? "?"} holder_source=${info?.source ?? "?"} age_ms=${Number.isFinite(age) ? Math.round(age) : "?"}`,
      );
      continue;
    }
    if (Date.now() >= deadline) {
      throw new Error(
        `另一进程正在写入本工作区(持有者 pid=${info?.pid ?? "?"},来源 ${info?.source ?? "?"},已持锁约 ${Math.round(age / 1000)}s)。` +
...
async function releaseWriteLock(repo: string): Promise<void> {
  const key = lockKey(repo);
  const held = heldWriteLocks.get(key);
  if (!held) return;
  held.depth -= 1;
...
function withWorkspace<T>(repo: string, tag: SourceLike, fn: () => Promise<T>): Promise<T> {
...
function ipActiveAt(ip: { lastActiveAt?: string; startedAt: string }): string {
  return ip.lastActiveAt ?? ip.startedAt;
}

function ipOwnerLine(ip: WsInProgress): string {
  const who = ip.ownerClient ? `${ip.ownerSource ?? "旧版工具"} · 工具 ${ip.ownerClient}` : (ip.ownerSource ?? "旧版工具");
  return (
    `另一会话(来源 ${who},开始于 ${ip.startedAt},最近活跃 ${ipActiveAt(ip)})` +
    `正在进行:需求「${ip.requirement}」`
  );
}

/** 收尾/回滚的所有权闸:令牌不匹配(或旧版登记无令牌)一律拒绝,堵住"替他人会话强行收尾"的路径 */
function requireOwnership(ip: WsInProgress, sessionToken: string | undefined): void {
  if (!ip.owner) {
    throw new Error(
      `${ipOwnerLine(ip)}。该进行中版本由旧版工具登记(无会话令牌),不能直接收尾:` +
        `请确认对方已停止后用 vcs_begin takeover="continue" 接管,再行处理`,
    );
  }
  if (sessionToken !== ip.owner) {
    throw new Error(
      `${ipOwnerLine(ip)}。请勿替其他会话收尾或回滚:等待其完成;` +
...
async function stashMain(repo: string, message: string): Promise<void> {
  await git(repo, ["stash", "push", "--include-untracked", "-m", message, "--", WS_MAIN]);
}

/** 短名:小写字母/数字/连字符,取自 name 或 requirement,长度受限 */
function slugify(input: string, fallbackSeed: string): string {
...
function assertNoMerge(reg: WsRegistry, op: string): void {
...
/** 从注册表找分支任务 */
function findBranch(reg: WsRegistry, name: string): WsBranchTask {
  const t = (reg.branches ?? []).find((b) => b.name === name);
  if (!t) {
    throw new Error(
      `并行分支不存在:${name}(用 vcs_branches 查看现有分支;分支名在创建时确定)`,
    );
  }
  return t;
}
...
function normAssignee(v: string | undefined): string | undefined {
  const s = (v ?? "")
    .toLowerCase()
    .replace(/[^a-z0-9._-]+/g, "")
    .slice(0, 40);
...
function branchAccess(
  ...
  tag: SourceLike,
): { via: "token" | "assigned" | "gui" } | { via: "third"; warn: string; deny: string } {
  const tg = normTag(tag);
  if (sessionToken !== undefined && sessionToken === t.token) return { via: "token" };
  if (tg.source === "gui") return { via: "gui" };
...
function requireBranchAccess(
  t: WsBranchTask,
  opts: { token?: string; strict?: boolean },
  tag: SourceLike,
): { via: "token" | "assigned" | "gui" | "third"; warn: string } {
  const r = branchAccess(t, opts.token, tag);
  if (r.via === "third") {
    if (opts.strict) throw new Error(r.deny);
    return r;
  }
...
function ownershipWarn(ownerClient: string | undefined, client: string | undefined): string {
  if (!client || !ownerClient || ownerClient === client) return "";
...
async function wsBeginUnlocked(
  repo: string,
  opts: WsBeginOptions,
  tag: SourceLike,
): Promise<WsBeginResult> {
  ...
  if (opts.takeover === "continue" && ip) {
    ...
    await wsAppendAudit(
      ...
      t,
      "ws_takeover",
      `mode=continue code=${ip.code} requirement="${ip.requirement.slice(0, 120)}"`,
    );
    return {
      token,
      message: [
  ...
  if (opts.takeover === "fresh" && ip) {
    ...
    await wsAppendAudit(
      ...
      t,
      "ws_takeover",
      `mode=fresh stashed_code=${ip.code} requirement="${ip.requirement.slice(0, 100)}"`,
    );
    reg.inProgress = null;
  }

  if (!opts.requirement.trim()) throw new Error("requirement 必填:记录用户对本轮修改的需求");
...
export function wsCommit(
  repo: string,
  opts: WsCommitOptions & { source?: SourceLike },
): Promise<string> {
  return withWorkspace(repo, opts.source ?? "api", () => wsCommitUnlocked(repo, opts));
}

async function wsCommitUnlocked(
  repo: string,
  opts: WsCommitOptions & { source?: SourceLike },
): Promise<string> {
  ...
  if (!opts.summary.trim()) throw new Error("summary 必填:本轮 AI 做了什么改动");

  // 强制约束:工作区内只允许三类产物
  const offenders = await wsOffenders(repo);
  if (offenders.length > 0) {
    throw new Error(
      `工作区存在三类产物之外的越界文件,拒绝提交(本工具约束 AI 只产生:main/、V X.Y.Z/、工作区日志.md):\n` +
  ...
  if (excludes.length > 0 && mainDirty.length === 0) {
    throw new Error(
      "按 exclude 排除后 main/ 已无内容变更,拒绝生成空版本;请调整 exclude 或先完成修改",
    );
  }
  if (dirty.length === 0) {
  ...
  const last = reg.versions[reg.versions.length - 1] ?? null;
  const bump: Bump | "major" = opts.bump ?? "content";
  ...
  if (
    ...
    throw new Error(
      `注册表已被其他进程修改(磁盘最新 ${fresh?.versions[fresh.versions.length - 1]?.code ?? "无"}),` +
  ...
  await fs.writeFile(
    path.join(vDir, "改动日志.md"),
    [`# 改动日志 —— ${code}(相对上一版 ${last?.code ?? "—"})`, "", `${opts.summary.trim()}`, ""].join("\n"),
    "utf8",
  );
  ...
  try {
    ...
    committed = true;
    if (await tagExists(repo, code)) throw new Error(`tag ${code} 已存在`);
...
async function collectTargetFiles(
  repo: string,
  target: WsVersion | undefined,
  code: string,
): Promise<CopyFile[]> {
  const tags = new Set(await tagList(repo));
  const isLocal = tags.has(code);
  const useOrigin =
    !isLocal && !!target?.originRepo && !!target.origin && !!target.originIncludes?.length;
...
async function writeCopy(repo: string, code: string, targetFiles: CopyFile[]): Promise<number> {
...
async function commitCopyHousekeeping(
  repo: string,
  code: string,
  message: string,
): Promise<string> {
...
async function wsRollbackUnlocked(
  repo: string,
  opts: {
    code: string;
    reason?: string;
  ...
  if (reg.inProgress) {
    ...
    if (opts.resolveInProgress !== "stash") {
      throw new Error(
        `${ipOwnerLine(ip)}。回滚前需先处理进行中版本:持有者先 vcs_commit 收尾;` +
    ...
    await wsAppendAudit(
      ...
      t,
      "ws_takeover",
      `mode=stash-for-rollback code=${ip.code} requirement="${ip.requirement.slice(0, 100)}"`,
    );
    reg.inProgress = null;
    await wsSaveRegistry(repo, reg);
  ...
  if (mainDirty.length > 0 && !opts.discardDirty && !stashedInProgress) {
    throw new Error(
      `主干 main/ 有 ${mainDirty.length} 处未收尾修改,回滚会覆盖;确认请传 discardDirty:true` +
  ...
  for (const rel of [...trackedNow, ...untrackedNow]) {
    if (want.has(rel.replaceAll("\\", "/"))) continue;
    const abs = path.resolve(repo, WS_MAIN, rel);
    if (abs.startsWith(root2 + path.sep)) await fs.rm(abs, { force: true });
  }
  // ③ 写入目标内容到 main/
  for (const f of targetFiles) {
  ...
  const sha = await commit(
    repo,
    `vcs: ${newCode} — 回滚到 ${code}`,
    paths,
  );
  if (await tagExists(repo, newCode)) throw new Error(`tag ${newCode} 已存在`);
...
export function wsDeleteCopy(repo: string, code: string, tag: SourceLike = "api"): Promise<string> {
  return withWorkspace(repo, tag, () => wsDeleteCopyUnlocked(repo, code, tag));
}

async function wsDeleteCopyUnlocked(
  repo: string,
  code: string,
  tag: SourceLike,
): Promise<string> {
  ...
  if (!target) throw new Error(`版本 ${v} 不存在`);
  ...
  const tags = new Set(await tagList(repo));
  const recoverable =
    tags.has(v) || (!!target.originRepo && !!target.origin && !!target.originIncludes?.length);
  if (!recoverable) {
...
export function wsRestoreCopy(repo: string, code: string, tag: SourceLike = "api"): Promise<string> {
  return withWorkspace(repo, tag, () => wsRestoreCopyUnlocked(repo, code, tag));
}

async function wsRestoreCopyUnlocked(
  repo: string,
  code: string,
  tag: SourceLike,
): Promise<string> {
  ...
  if (exists) throw new Error(`版本 ${v} 已有项目副本,无需重复生成(如需重建请先删除)`);
...
export function wsBranchBegin(
  repo: string,
  opts: WsBranchBeginOptions,
  tag: SourceLike = "api",
): Promise<WsBeginResult & { branch: string; worktree: string }> {
  return withWorkspace(repo, tag, () => wsBranchBeginUnlocked(repo, opts, tag));
}

async function wsBranchBeginUnlocked(
  repo: string,
  opts: WsBranchBeginOptions,
  tag: SourceLike,
): Promise<WsBeginResult & { branch: string; worktree: string }> {
  ...
  let name = base;
  for (let i = 2; reg.branches.some((b) => b.name === name); i++) name = `${base}-${i}`;
  ...
  if (opts.base?.trim()) {
    ...
    if (!ok.trim()) throw new Error(`基点 ${ref} 不存在:请给有效的 V 版本号(如 V1.10.0)或 commit sha`);
    baseRef = ref;
  }
  ...
  await wsAppendAudit(
    ...
    t,
    "ws_branch_begin",
    `name=${name} branch=${branchName} baseline=${baseline.slice(0, 10)}${assignedClient ? ` assigned=${assignedClient}` : ""} requirement="${opts.requirement.trim().slice(0, 100)}"`,
  );
  return {
    token,
    branch: branchName,
...
export function wsBranchCommit(
  repo: string,
  opts: { name: string; token?: string; summary: string; strict?: boolean },
  tag: SourceLike = "api",
): Promise<string> {
  return withWorkspace(repo, tag, () => wsBranchCommitUnlocked(repo, opts, tag));
}

async function wsBranchCommitUnlocked(
  repo: string,
  opts: { name: string; token?: string; summary: string; strict?: boolean },
  tag: SourceLike,
): Promise<string> {
  ...
  if (dirty.length === 0) throw new Error("工作树无修改,拒绝空检查点");
  await addPaths(wt, ["."]);
...
export function wsBranchMerge(
  repo: string,
  opts: { name: string; token?: string; summary?: string; abort?: boolean; strict?: boolean; confirmStaleBase?: boolean },
  tag: SourceLike = "api",
): Promise<string> {
  return withWorkspace(repo, tag, () => wsBranchMergeUnlocked(repo, opts, tag));
}

async function wsBranchMergeUnlocked(
  repo: string,
  opts: { name: string; token?: string; summary?: string; abort?: boolean; strict?: boolean; confirmStaleBase?: boolean },
  tag: SourceLike,
): Promise<string> {
  ...
  if (reg.mergeInProgress) {
    if (reg.mergeInProgress.name !== t.name) {
      throw new Error(
        `另一分支 ${reg.mergeInProgress.name} 的合并冲突尚未处理完(单槽):先完成或放弃它`,
      );
    }
    const acc = requireBranchAccess(t, opts, tag);
    ...
    if (offenders.length > 0) {
      throw new Error(
        `工作区存在越界文件,拒绝合并定版(全量提交会把它们卷入版本历史):\n${offenders.slice(0, 10).map((p) => `  ${p}`).join("\n")}\n` +
          `先移出工作区或纳入 main/ 后重试`,
      );
    }
  ...
  if (reg.inProgress) {
    throw new Error(
      `主工作区有进行中的版本(${reg.inProgress.code}):先由持有者 vcs_commit 收尾再合并(合并点串行化,避免混入半成品)`,
    );
  }
  ...
  if (wtDirty.length > 0) {
    throw new Error(
      `分支 ${t.name} 的工作树有 ${wtDirty.length} 处未提交修改,合并定版将删除工作树、这些修改会丢失。` +
    ...
    if (mb) {
      ...
      if (baseCode) {
        ...
        if (rollbackInBetween.length > 0 && !opts.confirmStaleBase) {
          throw new Error(
            `检测到分支 ${t.name} 分叉后主干发生过回滚(${rollbackInBetween.map((v) => `${v.code}:${v.purpose}`).join("、")}):` +
  ...
  if (!mergeOk) {
    const conflicts = (await git(repo, ["diff", "--name-only", "--diff-filter=U"])).trim();
    if (!conflicts) throw new Error("合并失败且无冲突记录,请检查 git 状态后重试");
    reg.mergeInProgress = {
      name: t.name,
      token: t.token,
...
async function finalizeBranchMerge(
  repo: string,
  reg: WsRegistry,
  t: WsBranchTask,
  summary: string | undefined,
  tag: SourceLike,
  acc?: { via: string; warn: string },
): Promise<string> {
  const tg = normTag(tag);
  ...
  await fs.writeFile(
    path.join(vDir, "改动日志.md"),
    [`# 改动日志 —— ${code}(相对上一版 ${last?.code ?? "—"})`, "", changes, ...checkpointLines, ""].join("\n"),
    "utf8",
  );
  await fs.appendFile(
    path.join(repo, WS_LOG),
  ...
  if (!mergeState) {
    ...
    if (mergeHead && tip && mergeHead !== tip) {
      ...
      throw new Error(
        `分支 ${t.name} 在本次合并开始后又产生了新提交:定版内容将是合并时刻的旧头。` +
...
export function wsBranchDiscard(
  repo: string,
  opts: { name: string; token?: string; force?: boolean },
  tag: SourceLike = "api",
): Promise<string> {
...
export function wsBranchAdopt(
  repo: string,
  opts: { name: string; requirement: string; assignee?: string },
  tag: SourceLike = "api",
): Promise<WsBeginResult & { branch: string; worktree: string }> {
  return withWorkspace(repo, tag, () => wsBranchAdoptUnlocked(repo, opts, tag));
}

async function wsBranchAdoptUnlocked(
  repo: string,
  opts: { name: string; requirement: string; assignee?: string },
  tag: SourceLike,
): Promise<WsBeginResult & { branch: string; worktree: string }> {
  ...
  if (!opts.requirement?.trim()) throw new Error("requirement 必填:记录该分支的用户需求");
  const raw = (opts.name ?? "").trim();
  const name = raw.replace(/^vcs\/task\//, "");
  if (!name || /[\/\\]/.test(name)) throw new Error(`分支名无效:${raw}(用 vcs_branches 查看 orphan 清单,或给短名)`);
  const branchName = `vcs/task/${name}`;
  reg.branches = reg.branches ?? [];
  ...
  if (!refExists) throw new Error(`分支 ${branchName} 不存在:只有仓库现存且未登记的 vcs/task/* 分支可导入`);
  const worktreeRel = `${WS_BRANCH_DIR}/${name}`;
  const worktreeAbs = path.join(repo, worktreeRel);
  await git(repo, ["worktree", "prune"]);
  await fs.rm(worktreeAbs, { recursive: true, force: true }).catch(() => {});
  ...
  if (!baseline) baseline = await head(repo);
  const token = newSessionToken();
  const assignedClient = normAssignee(opts.assignee);
  reg.branches.push({
    name,
...
export function wsBranchRollback(
  repo: string,
  opts: { name: string; to?: string; token?: string; force?: boolean; strict?: boolean },
  tag: SourceLike = "api",
): Promise<string> {
  return withWorkspace(repo, tag, () => wsBranchRollbackUnlocked(repo, opts, tag));
}

async function wsBranchRollbackUnlocked(
  repo: string,
  opts: { name: string; to?: string; token?: string; force?: boolean; strict?: boolean },
  tag: SourceLike,
): Promise<string> {
  ...
  let target = "";
  if (!opts.to?.trim()) {
    if (checkpoints.length < 2) throw new Error(`分支 ${b.name} 没有可回退的上一检查点`);
    ...
    if (/^\d+$/.test(to)) {
      ...
      if (!target) throw new Error(`检查点 ${to} 不在分支 ${b.name} 的最近 ${checkpoints.length} 个提交内`);
    }
  }
  const wtDirty = await statusPorcelain(wt, ["."]);
  if (wtDirty.length > 0 && !opts.force) {
    throw new Error(
      `分支 ${b.name} 的工作树有 ${wtDirty.length} 处未提交修改,回退会丢弃它们;确认请带 force:true`,
    );
  }
  ...
  await wsAppendAudit(
    ...
    tg,
    "ws_branch_rollback",
    `name=${b.name} to=${target.slice(0, 10)} via=${acc.via}${wtDirty.length > 0 ? ` discarded_dirty=${wtDirty.length}` : ""}`,
  );
  return [
...
export function wsBranchUpdate(
  repo: string,
  opts: { name: string; token?: string; strict?: boolean },
  tag: SourceLike = "api",
): Promise<string> {
  return withWorkspace(repo, tag, () => wsBranchUpdateUnlocked(repo, opts, tag));
}

async function wsBranchUpdateUnlocked(
  repo: string,
  opts: { name: string; token?: string; strict?: boolean },
  tag: SourceLike,
): Promise<string> {
  ...
  if (wtDirty.length > 0) {
    throw new Error(
      `分支 ${b.name} 的工作树有 ${wtDirty.length} 处未提交修改:先 vcs_branch_commit 存检查点再更新`,
    );
  }
...
async function wsBranchDiscardUnlocked(
  repo: string,
  opts: { name: string; token?: string; strict?: boolean; force?: boolean },
  tag: SourceLike,
): Promise<string> {
  const tg = normTag(tag);
  const reg = await wsRequire(repo);
  ...
  if (wtDirty.length > 0 && !opts.force) {
    throw new Error(
      `分支 ${t.name} 的工作树有 ${wtDirty.length} 处未提交修改,丢弃将直接销毁它们。` +
  ...
  await wsAppendAudit(
    ...
    tg,
    "ws_branch_discard",
    `name=${t.name} branch=${t.branch} via=${acc.via}${wtDirty.length > 0 ? ` discarded_dirty=${wtDirty.length}` : ""}`,
  );
  return `已丢弃并行分支 ${t.name}(工作树与分支 ref 已删除;其上的未合并修改不可恢复,审计已留痕)`;
}

/** 改派/清空分支的管理工具(assignedClient);属主闸与 commit/merge/discard 一致 */
export function wsBranchAssign(
  repo: string,
  opts: { name: string; assignee?: string; token?: string; strict?: boolean },
  tag: SourceLike = "api",
): Promise<string> {
  return withWorkspace(repo, tag, () => wsBranchAssignUnlocked(repo, opts, tag));
}

async function wsBranchAssignUnlocked(
  repo: string,
  opts: { name: string; assignee?: string; token?: string; strict?: boolean },
  tag: SourceLike,
): Promise<string> {
  ...
  await wsAppendAudit(
    ...
    tg,
    "ws_branch_assign",
    `name=${b.name} assignee=${assigned ?? "clear"}${prev ? `(原 ${prev})` : ""} via=${acc.via}`,
  );
  return [
    assigned
...
export function wsTempSave(
  repo: string,
  opts: { summary: string; name?: string; sessionToken?: string },
  tag: SourceLike = "api",
): Promise<string> {
  return withWorkspace(repo, tag, () => wsTempSaveUnlocked(repo, opts, tag));
}

async function wsTempSaveUnlocked(
  repo: string,
  opts: { summary: string; name?: string; sessionToken?: string },
  tag: SourceLike,
): Promise<string> {
  ...
  if (!opts.summary?.trim()) throw new Error("summary 必填:为什么无法提交、这些变化是什么");
  // 有进行中版本时,main/ 变化属于持有者:仅持有者本人可放弃并转存 temp
  if (reg.inProgress) {
    const ip = reg.inProgress;
    if (!ip.owner || opts.sessionToken !== ip.owner) {
      throw new Error(
        `${ipOwnerLine(ip)}。main/ 的当前变化归持有者负责,他人不得转存 temp;` +
  ...
  if (entries.length === 0) throw new Error("main/ 无未提交修改,无需保存 temp");
  const base = slugify(opts.name ?? opts.summary, "temp");
  reg.temp = reg.temp ?? [];
  let name = base;
  for (let i = 2; reg.temp.some((t) => t.name === name); i++) name = `${base}-${i}`;
  ...
  for (const rel of copied) {
    ...
    try {
      ...
      if (!inHead) await fs.rm(abs, { force: true });
    } catch {
      /* 文件本就不存在 */
    }
  }
...
async function wsTempApplyUnlocked(
  repo: string,
  opts: { name: string },
  tag: SourceLike,
): Promise<string> {
  ...
  if (!item) throw new Error(`temp 条目不存在:${opts.name}(用 vcs_branches 查看待处理清单)`);
  const tempDir = path.join(repo, WS_TEMP_DIR, item.name);
  let manifest: { copied: string[]; deleted: string[] };
  try {
  ...
  if (reg.inProgress) {
    throw new Error(
      `主工作区有进行中的版本(${reg.inProgress.code}):temp 恢复会与它叠加,先由持有者收尾,或确认后用 vcs_begin takeover 处理`,
    );
  }
...
async function wsTempDiscardUnlocked(
  repo: string,
  opts: { name: string },
  tag: SourceLike,
): Promise<string> {
  const reg = await wsRequire(repo);
  const item = (reg.temp ?? []).find((t) => t.name === opts.name);
  if (!item) throw new Error(`temp 条目不存在:${opts.name}`);
  reg.temp = (reg.temp ?? []).filter((t) => t.name !== item.name);
  await wsSaveRegistry(repo, reg);
...
export interface WsParallelView {
  ...
  temps: WsTempItem[];
  mergeInProgress: (WsMergeInProgress & { conflicts: string[] }) | null;
  /** 仓库现存但未登记的 vcs/task/* 分支(可经 vcs_branch_adopt 导入管理) */
  orphans: string[];
}

/** GUI 与 vcs_branches 共用:并行分支 + temp + 合并冲突状态的结构化清单 */
export async function wsParallelView(repo: string): Promise<WsParallelView> {
  const reg = await wsLoadRegistry(repo);
  if (!reg) throw new Error(`${repo} 不是工作区:先 vcs_init`);
  const branches: WsBranchTaskView[] = [];
  let cacheDirty = false;  for (const t of reg.branches ?? []) {
    let commits = 0;
    let checkpoints: WsBranchCheckpoint[] = [];
    try {
    ...
    try {
      ...
      if (mb) {
        const d = (await git(repo, ["describe", "--abbrev=0", "--match=V*", mb])).trim();
        if (/^V\d+\.\d+\.\d+$/.test(d)) baseVersion = d;
      }
    } catch {
      /* 无法定位分叉版本(如无任何 V tag) */
    }
  ...
  let mergeInProgress: WsParallelView["mergeInProgress"] = null;
  if (reg.mergeInProgress) {
  ...
  if (cacheDirty) await wsSaveRegistry(repo, reg); // 检查点缓存落库(原子写,随下次定版收编)
...
export async function wsPublishBranches(repo: string): Promise<WsPublishBranchView[]> {
  const refs = (await git(repo, ["branch", "--list", "publish/*"]))
    .split("\n")
    .map((l) => l.trim().replace(/^[*+]\s*/, ""))
    .filter(Boolean);
  refs.sort();
  const out: WsPublishBranchView[] = [];
...
export async function wsStatus(repo: string): Promise<string> {
  const reg = await wsLoadRegistry(repo);
  if (!reg) {
  ...
  if (latestCode) {
    try {
      unregistered =
        parseInt(
          (await git(repo, ["rev-list", "--count", `${latestCode}..HEAD`, "--", WS_MAIN])).trim(),
          10,
        ) || 0;
    } catch {
      /* 最新 tag 缺失等情况,忽略 */
...
function stallMinutesOf(ip: { lastActiveAt?: string; startedAt: string }): number {
  const t = Date.parse(ipActiveAt(ip));
  if (Number.isNaN(t)) return 0;
  return Math.max(0, Math.round((Date.now() - t) / 60_000));
}

function inProgressLine(ip: WsInProgress): string {
...
export async function wsTouch(
  ...
  return withWorkspace(repo, tag, async () => {
    const reg = await wsLoadRegistry(repo);
    const ip = reg?.inProgress;
    if (!reg || !ip || !ip.owner || ip.owner !== sessionToken) return false;
    ip.lastActiveAt = nowStr();
    await wsSaveRegistry(repo, reg);
    return true;
  });
...
export async function wsInProgressView(repo: string): Promise<WsInProgressView | null> {
  const reg = await wsLoadRegistry(repo);
  const ip = reg?.inProgress;
  if (!reg || !ip) return null;
...
export function occupancyOfRegistry(reg: WsRegistry | null): WsOccupancyView {
  if (!reg) return { inProgress: null, branchCount: 0, tempCount: 0, mergeConflict: false };
  const ip = reg.inProgress;
...
export async function wsListVersions(repo: string): Promise<WsVersionView[]> {
  const reg = await wsRequire(repo);
  const out: WsVersionView[] = [];
...
export async function wsReadDoc(
  repo: string,
  code: string,
  which: "purpose" | "changes" | "workspaceLog",
): Promise<string> {
...
export async function wsListFiles(repo: string, ref: string): Promise<string[]> {
  if (ref === "work") {
...
export async function wsDiffFiles(
  repo: string,
  from: string,
  to: string,
): Promise<WsDiffFileEntry[]> {
  ...
  return raw
    ...
    .map((l) => {
      ...
      return { file, status: (c === "A" || c === "D" ? c : "M") as "A" | "M" | "D" };
    })
    .filter((e) => e.file)
 ...
 */
export function wsMigrateLegacy(
  newRepo: string,
  opts: MigrateOptions,
): Promise<string> {
  return withWorkspace(newRepo, opts.source ?? "api", () =>
    wsMigrateLegacyUnlocked(newRepo, opts),
  );
}

async function wsMigrateLegacyUnlocked(
  newRepo: string,
  opts: MigrateOptions,
): Promise<string> {
  const oldRaw = await fs
    ...
    .catch(() => null);
  if (!oldRaw) throw new Error(`旧库不存在或无 paper_versions/paper-version.json:${opts.oldRepo}`);
  const old = JSON.parse(oldRaw) as {
    title: string;
    versions: { code: string; summary: string; note?: string; date?: string }[];
    paths: { cn?: string[]; en?: string[]; tex?: string[]; response?: string[]; extra?: string[] };
  };
  const includes = [
    ...(old.paths.cn ?? []),
    ...(old.paths.en ?? []),
  ...
  const tracked = (
    await git(opts.oldRepo, ["ls-files", "--", ...includes])
  )
    .split("\n")
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  const roundOf = (code: string) => (/^TEX1\./.test(code) ? 1 : 0);
  ...
  for (let i = 0; i < old.versions.length; i++) {
    ...
    let oldLog = "";
    try {
      oldLog = await fs.readFile(
        path.join(opts.oldRepo, "paper_versions", "changelogs", `${v.code}.md`),
        "utf8",
      );
    } catch {
      /* 旧 changelog 缺失 */
...
export interface WsAnalyzeReport {
  ...
  topEntries: { name: string; type: "dir" | "file"; bytes: number }[];
  plan: string[];
}
...
export async function wsAnalyze(dir: string): Promise<WsAnalyzeReport> {
  const st = await fs.stat(dir).catch(() => null);
  if (!st || !st.isDirectory()) throw new Error(`文件夹不存在:${dir}`);
  const reg = await wsLoadRegistry(dir);
  const byType = new Map<string, { count: number; bytes: number }>();
  const topEntries: { name: string; type: "dir" | "file"; bytes: number }[] = [];
  let totalFiles = 0;
  let totalBytes = 0;
  ...
  async function walk(d: string, depth: number): Promise<void> {
    ...
    for (const it of items) {
      ...
      if (it.isDirectory()) {
        if (depth === 0) {
          ...
          while (sub.length) {
            ...
            for (const e of ents) {
              ...
              if (e.isDirectory()) sub.push(cp);
              else bytes += (await fs.stat(cp).catch(() => null))?.size ?? 0;
            }
          }
        ...
        totalFiles += 1;
        totalBytes += b;
        if (depth === 0) topEntries.push({ name: it.name, type: "file", bytes: b });
